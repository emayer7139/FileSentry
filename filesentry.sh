...
# ─── Banner via Asciified ───────────────────────────────────────────────────────

# 1. Grab the JSON list of fonts
fonts_json=$(curl -s https://asciified.thelicato.io/api/v2/fonts)
...
# 3. Fallback if no fonts
(( ${#fonts[@]} )) || fonts=(standard)

# 4. Pick one & URL-encode
...
# 5. Show the banner
curl -s "https://asciified.thelicato.io/api/v2/ascii?text=${text_enc}&font=${font_enc}"
echo
echo "                            A file integrity monitor."
echo
